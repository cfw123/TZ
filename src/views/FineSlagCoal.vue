<template>
  <div class="view-container">
    <h2 class="view-title">细渣提精煤卸输煤台账</h2>
...
// 稳定锚点：进入编辑时锁定该行原始月份键，编辑期间行始终归属该月桶，
// 不因日期被清空/破坏而跳桶，直到 exitEditMode 提交时才重新排序。
...
// 桶构建期,把编辑中的行(清空/破坏日期)按快照日期锁定在原月桶。
// 不变量:editingId 进入编辑时锁定该行原始月份键,编辑期间行始终归属该月桶,
// 不因日期被清空/破坏而跳桶,直到 exitEditMode 提交时才重新排序。
const monthBuckets = computed(() => {
  const map = new Map()
  rows.value.forEach((row, idx) => {
    // 编辑中的行始终使用快照月份键进行桶分配，保证日期被清空时行仍留在原桶
...
function pageIndexOfRow(rowId) {
  const buckets = monthBuckets.value
  // 如果该行正在编辑中，用快照月份键查找对应桶
  if (editingId.value === rowId && snapshotMonthKey.value !== null) {
    const idx = buckets.findIndex((b) => b.key === snapshotMonthKey.value)
    return idx >= 0 ? idx : 0
  }
...
async function initData() {
  try {
    /**
     * GET /api/v1/fine-slag-coal
     * @param {string} table  'fine_slag_coal_rows'
     * @returns {FineSlagCoalRow[]}  记录数组；若数据库为空返回 []
     * @throws 网络错误时 catch 捕获，rows.value 设为 []
     */
    const data = await api.list('fine_slag_coal_rows')
    ...
    console.error('[FineSlagCoal] Failed to load data:', e)
    rows.value = []
  }
}
...
// 全量持久化（手动点「保存」时触发）
/**
 * DELETE /api/v1/fine-slag-coal
 * POST  /api/v1/fine-slag-coal/bulk
...
async function persist() {
  try {
    ...
    await api.clear('fine_slag_coal_rows')
    await api.bulkPut('fine_slag_coal_rows', payload)
    dirty.value = false
    lastSavedSignature.value = computeCurrentSignature()
    return true
  } catch (e) {
...
/**
 * PUT    /api/v1/fine-slag-coal/:id
 * POST   /api/v1/fine-slag-coal
...
async function dbUpsertRow(row) {
  ...
  if (row.id) {
    await api.update('fine_slag_coal_rows', row.id, clean)
  } else {
    const newItem = await api.create('fine_slag_coal_rows', clean)
    row.id = Array.isArray(newItem) ? newItem[newItem.length - 1].id : newItem.id
  }
}

/**
 * DELETE /api/v1/fine-slag-coal/:id
 * @description 按主键 id 删除单条记录；若 row.id 不存在则跳过。
 */
async function dbDeleteRow(row) {
  if (row.id) await api.remove('fine_slag_coal_rows', row.id)
}
...
function handleDelete(id) {
  ...
  confirmDialog.value = {
    ...
    bodyRows: [
      ...
      { label: '细渣提精煤来煤量', value: row.inbound || '—' },
      { label: '细渣提精煤总存煤量', value: row.stockTotal || '—' },
    ],
...
function handleExport() {
  ...
  const header = ['序号', '日期', '掺烧煤仓', '锅炉当日掺烧细渣量', '锅炉当月掺烧细渣量', '锅炉当年掺烧细渣量', '细渣提精煤来煤量', '207A存煤量', '207B存煤量', '细渣提精煤总存煤量']
  ...
  a.href = url
  a.download = `细渣提精煤卸输煤台账_${today}.csv`

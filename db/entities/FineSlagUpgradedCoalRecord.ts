...
/**
 * 原运装置细渣提精煤卸输煤记录（卸输煤过磅登记表）
 * 对应中文台账：序号 / 日期 / 车号 / 车型 / 货名 / 发货单位 / 收货单位 /
 *              发运数量(吨) / 实收数量(吨) / 亏吨数量(吨) / 亏吨率(%) /
 *              司磅员 / 卸车地点 / 备注
 */
@Entity({ name: 'fine_slag_upgraded_coal_record', comment: '原运装置细渣提精煤卸输煤记录' })
@Index('idx_fine_slag_record_date', ['recordDate'])
export class FineSlagUpgradedCoalRecord {
